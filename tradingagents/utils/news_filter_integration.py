...
def integrate_news_filtering(original_get_stock_news_em):
    ...
    def filtered_get_stock_news_em(symbol: str, enable_filter: bool = True, min_score: float = 30, 
        ...
        Args:
            ...
            min_score: 最低相关性评分阈值
            use_semantic: 是否使用语义相似度过滤（预留）
            use_local_model: 是否使用本地分类模型（预留）
        ...
        try:
            ...
            try:
                ...
                # 创建过滤器
                news_filter = create_news_filter(symbol)
                
                # 执行过滤
                filtered_df = news_filter.filter_news(news_df, min_score=min_score)
               ...
               (not '.' in ticker and ticker.isdigit()):
                ...
                try:
                    ...
                    if not filtered_news_df.empty:
                        ...
                        report = f"# {ticker} 智能过滤新闻报告\n\n"
                        report += f"📅 生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                        report += f"📊 过滤后新闻总数: {news_count}条\n"
                        report += f"🔍 过滤阈值: {min_score}分\n"
                        
                        # 添加过滤统计信息
                        if enable_filter and 'final_score' in filtered_news_df.columns:
                        ...
                        for idx, (_, row) in enumerate(filtered_news_df.iterrows()):
                            ...
                            report += f"📅 发布时间: {row.get('发布时间', '无时间')}\\n"
                            
                            if 'final_score' in row and enable_filter:
                                ...
                                if score >= 70:
                                    ...
                                    score_emoji = "💡"
                                report += f"{score_emoji} 相关性评分: {score:.1f}分\\n"
                            
                            report += f"🔗 [查看原文]({row.get('新闻链接', '#')})\\n\\n"
                        ...
                        logger.info(f"[增强实时新闻] ✅ 成功生成智能过滤新闻报告，包含 {news_count} 条高质量新闻")
                        return report
                    else:
...
def apply_news_filtering_patches():
    ...
    自动应用新闻过滤补丁
    """
    logger.info("[新闻过滤集成] 🚀 开始应用智能新闻过滤补丁...")
    ...
    logger.info("[新闻过滤集成] ✅ 智能新闻过滤补丁应用完成")
